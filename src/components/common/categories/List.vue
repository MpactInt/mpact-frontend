    ...
    <section>
        <div class="py-6 flex justify-between px-8">
            <p class="uppercase text-4xl text-gray-400 dark:text-gray-500 uppercase font-bold">
                <span class="text-[#090446]">Categories</span>
            </p>
        </div>
        <!-- card-10 stat -->
        <div class="w-full">
            <div class="py-6 px-8">
                <div class="flex justify-between py-6">
                    <form class="flex items-center">
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" id="simple-search"
                                ...
                                placeholder="Search" required v-model="getCategoryData.keyword" v-on:keyup="getCategoriesList">
                        </div>
                    </form>
                    ...
                    <div class="flex flex-row space-x-1">
                        <button
                            class="flex items-center font-sixe-[20px] px-8 py-2 rounded-md bg-[#0A0446] text-white text-center text-md shadow" v-b-modal.add-category-modal>
                            ...
                            <span class="flex-1 text-white ml-3 whitespace-nowrap" v-b-modal.add-category-modal>Add New Category</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="relative overflow-x-auto shadow-md">
                    <table class="w-full text-sm text-center text-white">
                        ...
                        <tbody class="text-[#090446]">
                            <tr class="bg-white hover:bg-gray-50" v-if="categoriesLength" v-for="r in categoriesList.data" v-bind:key="r.id">
                                <td scope="row"
                                ...
                                <td class="px-6 py-4 border-r border-gray-300">
                                    <img :src="filePath + '/' + r.category_image" class="table-img" height="70" width="70" />
                                </td>
                                <td
                                    class="px-6 py-4 border-r border-gray-300 flex items-center justify-around space-x-2 px-1 border-r border-gray-300">
                                    <button
                                        class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md font-medium shadow border-2" v-b-modal.update-category-modal @click="getCategory(r.id)">
                                    ...
                                    <button
                                        class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md shadow border-2" @click="deleteCategory(r.id)">
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                            ...
                            <tr class="bg-white hover:bg-gray-50" v-if="!categoriesLength">
                                <td scope="row"
                                    class="px-6 py-4 border-r border-gray-300 font-medium whitespace-nowrap" colspan="4">No Data Found</td>
                            </tr>
                        </tbody>
                    </table>
                    <pagination :data="categoriesList" @pagination-change-page="getCategoriesList" />
                </div>
            </div>
        </div>
        <!-- card-10 end -->
        ...
        <Add :getCategoriesList="getCategoriesList"></Add>
        <!--Update category modal popup-->
        <Edit :getCategoriesList="getCategoriesList" :categoryUpdate="categoryUpdate"></Edit>
...
import Add from '../categories/Add.vue'
...
export default {
  ...
  methods: {
    getCategoriesList: function (page = 1) {
      let that = this
      Api.getCategoriesList(page, that.getCategoryData).then(response => {
        let that = this
        that.categoriesList = response.data.res
        that.categoriesLength = that.categoriesList.data.length
    ...
    getCategory: function (id) {
      ...
      Api.getCategory(id).then(response => {
        // that.$bvModal.show('update-category-modal')
        that.categoryUpdate.id = response.data.res.id
        that.categoryUpdate.category_name = response.data.res.category_name
        that.categoryUpdate.category_desc = response.data.res.category_desc
        that.categoryUpdate.showFile = response.data.res.category_image
      })
    },
  },
  mounted() {

    ...
    <section>
        <div class="py-6 flex justify-between px-8">
            <p class="uppercase text-4xl text-gray-400 dark:text-gray-500 uppercase font-bold">
                <span class="text-[#090446]">Tips</span>
            </p>
        </div>
        <div class="w-full">
            <div class="py-6 px-8">
                <div class="flex justify-between py-6">
                    ...
                    <div class="flex flex-row space-x-1">
                        <button
                            ...
                            <span class="flex-1 text-white ml-3 whitespace-nowrap" v-b-modal.add-tip-modal>Add Tip</span>
                        </button>
                    </div>
                </div>
                        ...
                        <tbody class="text-[#090446]">
                            <tr class="bg-white hover:bg-gray-50" v-if="tipsLength" v-for="r in tips.data" v-bind:key="r.id">
                                ...
                                <td
                                    ...
                                    <button
                                        class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md font-medium shadow border-2" v-if="user.role == 'ADMIN'" @click="tipId.tip_id=r.id; getTipById()" v-b-modal.update-tip-modal>
...
export default {
  ...
  methods: {
    ...
    getTipsList: function (page = 1) {
      let that = this
      Api.getTipsList(that.getTipsData, page).then(response => {

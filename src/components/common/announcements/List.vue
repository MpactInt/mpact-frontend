<template>
<section>
    <div class="w-full flex md:flex-wrap justify-between px-8" v-if="company.role=='COMPANY_ADMIN'">
        <div class="flex md:flex-wrap md:w-full justify-between py-3">
                ...
                <form>
                    ...
                    <div class="relative w-full">
                        ...
                        <input type="text" id="simple-search"
                            ...
                            placeholder="Search" required v-model="searchData.keyword" v-on:keyup="getAnnouncementsList">
                    </div>
                </form>
                <!-- Main modal -->
            </div>
            ...
            <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal"
                class="m-2 flex py-2 space-x-2 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg" v-b-modal.add-announcement-modal>
                <svg class="h-5 pl-3" xmlns="http://www.w3.org/2000/svg" version="1.1"
                    ...
                    <g>
                        <path
                            ...
                            fill="#2c3e50" data-original="#000000"  opacity="1"></path>
                        <path
                            ...
                            fill="#2c3e50" data-original="#000000" class="" opacity="1"></path>
                    </g>
                </svg>
                <span class="pr-3"> Add Announcement</span>
            </button>
        </div>
    </div>
    <!-- card-10 stat -->
    <div class="w-full px-8">
        <form class="my-4" v-if="company.role=='COMPANY_EMP'"> 
            <label for="simple-search" class="sr-only">Search</label>
            <div class="relative w-full">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <svg aria-hidden="true" class="w-5 h-5 text-gray-500" fill="currentColor"
        ...
        <!-- card-8 stat -->
        <div class="flex p-[20px] space-x-4 border border-1 border-gray-100 rounded-[18px] my-4 bg-white"  v-if="announcementsLength" v-for="a in announcementList.data" v-bind:key="a.id">
            <div class="flex items-center space-x-4 w-full">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex flex-wrap justify-between items-center">
                        <div>
                            <h2 class="font-bold tracking-tight">
                                <span class="text-[#BE0858]">{{ a.title }}</span>
                            </h2>
                        </div>
                        &nbsp;
                        <div class="inline-flex space-x-4 items-center" v-if="company.role == 'COMPANY_ADMIN'">
                            <button  @click="getAnnouncement(a.id)">
                            ...
                            <button @click="deleteAnnouncement(a.id)" v-if="!a.deleted_at">
                                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                    ...
                    <p class="text-sm text-justify text-gray-500">{{ a.description }}</p>
                    <p class="inline-flex my-2">
                        ...
                        <span>Date:&nbsp;</span>
                        <span class="font-bold">{{ a.date | timeStampToDateOnly }} {{a.date | timeStampToTimeOnly}}</span>
  ...
  methods: {
    getAnnouncementsList: function (page = 1) {
      ...
      Api.getAllAnnouncementsList(that.company.id, that.searchData, page).then(response => {
        ...
        for (const key in response.data.res.data){
            response.data.res.data[key].date = new Date(response.data.res.data[key].date * 1000);
        }
       
        that.announcementList = response.data.res
        that.announcementsLength = that.announcementList.data.length
        //new Date(moment.unix(response.data.res.date).format('YYYY-MM-DD HH:mm:ss'))

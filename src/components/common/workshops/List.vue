  ...
  <section>
   <div class="py-6 flex justify-between px-8">
        <p class="uppercase text-4xl text-gray-400 dark:text-gray-500 uppercase font-bold">
            <span class="text-[#090446]">Workshops</span> <span class="text-[#090446]">Workshops</span>
        </p>
    </div>
        ...
        <div class="py-6 px-8">
            <div class="flex justify-between py-6">
                <form class="flex items-center">
                    ...
                    <div class="relative w-full">
                        ...
                        <input type="text" id="simple-search"
                            ...
                            placeholder="Search" required v-model="getWorkshopData.keyword" v-on:keyup="getWorkshopsList">
                    </div>
                </form>
                ...
                <div class="flex flex-row space-x-1">
                    <button
                        class="flex items-center font-sixe-[20px] px-8 py-2 rounded-md bg-[#0A0446] text-white text-center text-md shadow" v-b-modal.add-workshop-modal>
                        ...
                        <span class="flex-1 text-white ml-3 whitespace-nowrap">Add New Workshop</span>
                    </button>
                </div>
            </div>
            ...
            <div class="relative overflow-x-auto shadow-md">
                <table class="w-full text-sm text-center text-white">
                    ...
                    <tbody class="text-[#090446]">
                        <tr class="bg-white hover:bg-gray-50" v-if="workshopsLength" v-for="r in workshopsList.data" v-bind:key="r.id">
                            <td class="px-6 py-4 border-r border-gray-300">
                                <span>{{r.company.map(({company_name})=>company_name).join(', ') }}</span> 
                            </td>
                            <td class="px-6 py-4 border-r border-gray-300">
                            ...
                            <td class="px-6 py-4 border-r border-gray-300">
                                <span v-html="r.additional_info"></span>
                            </td>
                            <td
                                class="px-6 py-4 border-r border-gray-300 flex items-center justify-around space-x-2 px-1 border-r border-gray-300">
                                <button
                                    class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md font-medium shadow border-2" @click="getCompaniesListMultiselect(); getWorkshop(r.id)">
                                ...
                                <button
                                    class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md shadow border-2" @click="deleteWorkshop(r.id)">
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                ...
                                <router-link :to="'/admin/view-workshop/' + r.id">
                                  <button class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md shadow border-2">

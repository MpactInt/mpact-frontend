<template>
<section>
    <section class="px-8 py-6"> </section>

    <section>
        <div class="flex justify-between px-8" id="tutorial_setup_guided-trans">
            <p class="uppercase text-2xl text-gray-400 dark:text-gray-500 uppercase font-bold">
                <span class="text-[#0A0446]">Guided Transformation</span>
            </p>
        </div>

        <div class="flex flex-wrap p-2 px-8 lg:space-x-4">

            <!-- Swiper -->
            <div class="swiper mySwiper lg:w-[68%] md:w-full" style="width:68%">
                <div class="swiper-wrapper">
                    <div class="swiper-slide mb-10" v-if="stepsList.length" v-for="(step, index) in stepsList" v-bind:key="step.id">
                        <!-- card-0 stat -->
                        <div class="border border-gray-200 bg-white shadow rounded-[20px] hover:bg-slate-100">
                            <div class="p-[15px]">
                                <a class="" href="#">
                                    <img class="object-cover rounded-[15px]" :src="filePath + '/' + step.image" alt="..."/>
                                </a>
                            </div>
                            <div class="px-5">
                                <a href="#">
                                    <h5 class="text-2xl font-semibold tracking-tight">Step {{ index }}</h5>
                                </a>
                                <p class="mt-2 text-sm text-gray-500  text-start">{{ step.title }}</p>
                            </div>
                                ...
                                <router-link :to="'/employer/view-step/' + step.id">
                                    <button
                                        ...
                                        <svg class="ml-2" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                            ...
                                            <path d="M5 12L19 12M19 12L12 5M19 12L12 19" stroke="white"
            ...
            <div
                class="lg:w-[30%] md:w-full bg-[#f7f7f7db] border border-gray-200 rounded-[20px] shadow text-black h-fit" id="upcoming-workshop">
                <p class="pt-2 text-center font-bold text-lg">
                    <span class="text-[#0A0446]">Upcoming Workshops</span>
                </p>
                <ul class="max-w-full space-y-1">
                    <li class="m-4 rounded-lg bg-[#f7f7f7db] hover:bg-slate-100" v-if="workshopsListDashboard.length" v-for="rl in workshopsListDashboard" v-bind:key="rl.id">
                        <router-link :to="'/employer/view-workshop/' + rl.id">
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0">
                                    <img class="h-10 rounded-md" :src="rl.image" alt="product image" />
                                </div>
                ...
                <div class="pt-2 pb-4">
                    <router-link to="/employer/workshops">
                        <button
                            class="flex items-center font-sixe-[20px] px-8 py-1 mx-auto rounded-md bg-[#0A0446] text-white text-center text-md border border-1 border-black">
                            See All
                        </button>
    ...
    <section>
        <div class="py-2 flex justify-between px-8" id="tutorial_setup_todo">
            <p class="uppercase text-2xl text-gray-400 dark:text-gray-500 uppercase font-bold">
                <span class="text-[#0A0446]">To-Do</span>
            </p>
        </div>
        ...
        <div class="flex flex-wrap p-2 px-8 lg:space-x-4">
            ...
            <div class="swiper mySwiper lg:w-[68%] md:w-full"  style="width:68%">
                <div class="swiper-wrapper">    
                    <div class="swiper-slide mb-10" v-if="todoList.length" v-for="todo in todoList" v-bind:key="todo.id">
                        <!-- card-5 stat -->
                        <div
                            ...
                            <div class="bg-[#F2F2F2] flex items-center space-x-4 px-8 py-6 rounded-t-[10px]">
                                <div class="flex-shrink-0">
                                    <svg class="h-10 w-10" viewBox="0 0 66 63" fill="none"
                                        ...
                                        <mask id="mask0_22_7635" style="mask-type:luminance"
                                            maskUnits="userSpaceOnUse" x="18" y="16" width="31" height="30">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="leading-6 text-start">
                                        {{todo.title | truncate(18)}}
                                    </p>
                                </div>
                            </div>
                            <div class="px-8 pt-6 text-left">
                                <p class="leading-6 text-start" v-html="todo.description.replace(/<\/?[^>]+(>|$)/g, '').slice(0, 80)"></p>
                                <p>
                                    <router-link :to="'/employer/todo/' + todo.id"  class="font-medium text-[#BE0858]">Read More...</a></router-link>
                                </p>
                            ...
                            <div class="pb-4">
                                <button
                                    ...
                                    {{ todo.status }}
                                    <svg class="ml-2" width="20" height="10" viewBox="0 0 20 10" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1 0.999579L10 8.71387L19 0.999578" stroke="white"
            ...
            <div
                class="lg:w-[30%] md:w-full bg-[#f7f7f7db] border border-gray-200 rounded-[20px] shadow text-black h-fit">
                <p class="pt-2 text-center font-bold text-lg">
                    <span class="text-[#0A0446]">Announcements</span>
                </p>
                <ul class="max-w-full space-y-1">
                    <li class="m-4 rounded-lg bg-[#f7f7f7db] hover:bg-slate-100" v-if="announcementsList.length" v-for="rw in announcementsList" v-bind:key="rw.id">
                        <div class="flex items-center space-x-4">
                            <svg width="52" height="49" viewBox="0 0 66 63" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <ellipse cx="33" cy="31.5" rx="33" ry="31.5" fill="#0A0446">
                                </ellipse>
    ...
    <section>
        <div class="py-2 flex justify-between px-8">
            <p class="uppercase text-2xl text-gray-400 dark:text-gray-500 uppercase font-bold">
                <span class="text-[#0A0446]">Resources</span>
            </p>
        </div>
        ...
        <div class="flex flex-wrap p-2 px-8 lg:space-x-4">
            ...
            <div class="swiper mySwiper lg:w-[68%] md:w-full"  style="width:68%">
                <div class="swiper-wrapper">
                    <div class="swiper-slide mb-10" v-for="rl in resourcesList" v-bind:key="rl.id">
                        <!-- card-0 stat -->
                        <div class="border border-gray-200 bg-white shadow rounded-[20px] hover:bg-slate-100">

                            <div class="p-[15px]">
                                <img class="rounded-[15px] resized-image"
                                        src="../../../assets/images/logo25dpi.png"
                                        alt="product image" />
                            </div>
                            <div class="px-5">
                                <h5 class="text-2xl font-semibold tracking-tight">{{ rl.title }}</h5>
                                
                                <p class="mt-2 text-sm text-gray-500  text-start">{{ rl.description }}</p>
                                <a
                                    href="https://mpact-int.com/at-a-glance/">https://mpact-int.com/at-a-glance/</a>
                            </div>
                            <div class="my-4 rounded">
                                <button
                                    class="flex items-center font-sixe-[20px] px-8 py-1 mx-auto rounded-md bg-[#0A0446] text-white text-center text-md border border-1 border-black" v-if="rl.file" @click="downloadFile(rl.id, rl.file)">
                                    Download File
                                    <svg class="ml-2" width="22" height="20" viewBox="0 0 22 20" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M20.7273 19H1.5M17.5227 9L11.1136 15M11.1136 15L4.70454 9M11.1136 15V1"
            ...
            <div
                class="lg:w-[30%] md:w-full bg-[#f7f7f7db] border border-gray-200 rounded-[20px] shadow text-black h-fit">
                <p class="py-2 text-center font-bold text-xl text-[#0A0446]">Chats</p>
                <ul class="max-w-full space-y-1 dark:divide-gray-700">
                    <router-link v-for="cl in chatList" v-bind:key="cl.id" :to="'/' + currentUrl + '/one-to-one-chat/' + cl.id" v-if="chatList.length" >
                        <li class="m-1 p-2 rounded-lg hover:bg-white">
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0">
                                    <img class="w-10 h-10 rounded-full" src="https://picsum.photos/400/300"
                                        alt="Neil image">
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-md font-medium font-black truncate">
                                        {{ cl.first_name }} {{ cl.last_name }} <span
                                            class="px-[6px] text-white pb-[2px] rounded-full bg-[#BE0858]">2</span>
                                    </p>
                ...
                <div class="pt-2 pb-4">
                     <router-link :to="'/' + currentUrl + '/one-to-one-chat/'" v-if="chatList.length" >
                        <button
                            class="flex items-center font-sixe-[20px] px-8 py-1 mx-auto rounded-md bg-[#0A0446] text-white text-center text-md border border-1 border-black">
                            See All
                        </button>
...
export default {
  ...
  methods: {
    initDriver() 
    {
        ...
        { element: '#tutorial_setup_setting', popover: { title: 'Settings', description: 'Navigate to “Settings” to update your company logo and create a personalized message for your team.  You can also change your password here.' } },
        { element: '#tutorial_setup_assessment', popover: { title: 'Assessment', description: 'Visit the assessment to complete your company profile and customize questions. Be sure to do this before you invite team members to join the platform. Be sure to check out “Resources” for a guide on “Getting Started with the Assessment”' } },
        { element: '#tutorial_setup_resources', popover: { title: 'Resources', description: 'Visit “Resources” for guides and other material to help you engage with your team.' } },
        { element: '#tutorial_setup_todo', popover: { title: 'To Do', description: 'To Do provides regular tips to help you maintain team momentum.' } },
        { element: '#tutorial_setup_managing-team-orga', popover: { title: 'Managing Your Team', description: 'To add team members, navigate to “Team and Organization Management”. Copy the link to send aninvitation to team members in bulk or you can manually add individual names. Team members will be sent an invitation to register.' } },
        { element: '#tutorial_setup_chat', popover: { title: 'Communication Features', description: 'We have a range of features to help you communicate with your employees.Use chat to send messages to individuals or to a group you create.To search by name type the name of the individual.' } },
        { element: '#tutorial_setup_feedback-team', popover: { title: 'Feedback from your team', description: 'You can review direct and anonymous feedback from employees by visiting the “Employee Feedback” section.' } },
        { element: '#tutorial_setup_dashboard', popover: { title: 'Dashboard', description: 'Use this link to access information about your team visits and engagement with the site.'} },
        { element: '#tutorial_setup_guided-trans', popover: { title: 'Guided Transformation ', description: 'This content provides you a step-by-step model to implement change management. To access the tools, click in each step, review the guidebook, and supporting material in the toolkit.' } },
        { element: '#tutorial_setup_custom-wr', popover: { title: 'Request A Custom Workshop', description: 'Custom workshop requests can be made by clicking this button and completing the form.' } },
        { element: '#tutorial_setup_custom-support', popover: { title: 'Support', description: 'To Get Support visit this link and complete the form.' } },
          ]
        });
    ...
    getAdminHomeSubTitle: function () {
      let that = this
      Api.getSetting('admin_home_sub_title').then(response => {
        let that = this
        that.dashboard_subtitle = response.data.res.value
      }
      ).catch((error) => {
        this.$swal({
          icon: "error",

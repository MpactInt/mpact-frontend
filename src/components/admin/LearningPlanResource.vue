    ...
    <section>
        <div class="py-6 flex justify-between px-8">
            <p class="uppercase text-4xl text-gray-400 dark:text-gray-500 uppercase font-bold">
                <span class="text-[#090446]">Learning Plan Resources</span>
            </p>
        </div>
                ...
                <div class="flex justify-between py-6">
                    <form class="flex items-center">
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" id="simple-search"
                                ...
                                placeholder="Search" required v-model="searchData.keyword" v-on:keyup="getLearningPlanResources">
                        </div>
                    </form>
                    ...
                    <div class="flex flex-row space-x-1">
                        <button
                            ...
                            <span class="flex-1 text-white ml-3 whitespace-nowrap" v-b-modal.add-modal>Add Learning Plan File</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="relative overflow-x-auto shadow-md">
                    <table class="w-full text-sm text-center text-white">
                        ...
                        <tbody class="text-[#090446]">
                            <tr class="bg-white hover:bg-gray-50" v-if="resourceListLength" v-for="lp in planFiles.data" v-bind:key="lp.id">
                                <td scope="row"
                                ...
                                <td class="px-6 py-4 border-r border-gray-300">
                                    <a class="cursor-pointer links"  @click="showLearningVdo(vdoPath + '/' + lp.video_path)" v-if="lp.video_path">Watch Video</a> <span v-else>NA</span>
                                </td>
                                <td class="px-6 py-4 border-r border-gray-300">
                                    <a v-if="lp.image" class="cursor-pointer links" @click="downloadLearningPlanFile(lp.id, lp.image)">Download</a> <span v-else>NA</span>
                                </td>
                                <td
                                    class="px-6 py-4 border-r border-gray-300 flex items-center justify-around space-x-2 px-1 border-r border-gray-300">
                                    <button
                                        class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md font-medium shadow border-2" @click="getLearningPlanFile(lp)">
                                    ...
                                    <button
                                        class="flex items-center px-3 py-1 rounded-md bg-white text-black text-center text-md shadow border-2" @click="deleteLearningPlanFile(lp.id)">
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                            ...
                            <tr class="bg-white hover:bg-gray-50" v-if="!resourceListLength">
                                <td scope="row" class="px-6 py-4 border-r border-gray-300 font-medium whitespace-nowrap"colspan="5">No Data Found</td>
                            </tr>
                        </tbody>
                    </table>
                    <pagination :data="planFiles" @pagination-change-page="getLearningPlanResources" />
                </div>
            </div>
        </div>
        <!-- card-10 end -->
            ...
            <form enctype="multipart/form-data">
                <div id="details">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="file" ref="addfileInputvideo" accept="video/*" @change="addUploadVideo"/>
                    </div>
        ...
        <b-modal id="update-modal" title="Update Learning Plan File" :hide-footer=hideFooter>
            <form enctype="multipart/form-data">
                <div id="details">
                    ...
                    <div class="form-group">
                        <label>Video Upload</label>
                        <input class="form-control" type="file" ref="fileInputvideo" accept="video/*" @change="uploadVideo"/>
                    </div>
                    <div class="form-group">
                        <label>File</label>
        ...
        <b-modal id="show-learning-vdo" title="Video" :hide-footer=hideFooter>
          ...
          <video id="vdo_pop_up" class="py-6" controls style="width: 100%;">
            <source :src="watchvideo" type="video/mp4" />Your browser does not support the video tag.
          </video>
        </b-modal>
...
export default {
    ...
    methods: {
        ...
        addUploadVideo: function (e)
        {
            let that = this
            this.plan.video_path = this.$refs.addfileInputvideo.files[0]
        },
        uploadVideo: function(e)
        {
            let that = this
            this.planUpdate.video_path = this.$refs.fileInputvideo.files[0]
        },
        ...
        addLearningPlanFile: function (e)
        {
            e.preventDefault()
            let that = this
            if (!that.plan.title || !that.plan.description )
            { //|| !that.plan.image
                this.$swal({
                    icon: 'error',
                    title: 'error',
            ...
            else
            {
                ...
                formData.append('video_path', that.plan.video_path)
        ...
        updateLearningPlanFile: function (e)
        {
            ...
            else
            {
                ...
                formData.append('video_path', that.planUpdate.video_path)
        ...
        getLearningPlanResources: function (page = 1) {
            let that = this
            Api.getLearningPlanResources(page, that.searchData).then(response => {
                that.planFiles = response.data.res
                that.filePath = response.data.path
                that.vdoPath = response.data.vdo_path
                that.resourceListLength = that.planFiles.data.length
            }).catch((error) => {
                this.$swal({
                    icon: "error",
                    title: "error",
        ...
        showLearningVdo: function (video_path)
        {
          let that = this
          that.$bvModal.show('show-learning-vdo')
          that.vdo_pop_up.video_path = video_path
          this.watchvideo = that.vdo_pop_up.video_path;
